...
## CREATING HIGH MODULARITY
Graph1_modular_h<-LFR(n=500, tau1=6, tau2=3, mu=0.1,min_community=50,max_community=100,average_degree=55, max_degree=80)
graph<-Graph1_modular_h$graph
...
plot(rand,
     ...
     edge.label.color = "black",   # Color of the edge labels
     edge.label.dist = 0.5,       # Distance of the labels from the edges
     main = "Null Model (Modularity=0.74)")
...
plot(graph, vertex.color=members_og, main="Graph Community (Modularity:0.74)", 
...
saveRDS(m1_df, file = "high_m1_df.rds")
saveRDS(m2_df, file = "high_m2_df.rds")
...
ggplot(means_df, aes(x = i_values)) +
  ...
  labs(
    title = "Stability Measure for Graph(Modularity=0.74)",
    x = "Perturbations(%)",
...
legend <- c("High Modularity Graph","null model")
