...
## CREATING Moderate MODULARITY 

graph<-LFR(n=500, tau1=6, tau2=1, mu=0.35,min_community=30,max_community=75,average_degree=18, max_degree=90)
graph<-graph$graph
...
plot(graph, vertex.color=members_og, main="Graph Community (Modularity:0.57)", 
...
saveRDS(m1_df, file = "moderate_m1_df.rds")
saveRDS(m2_df, file = "moderate_m2_df.rds")
...
ggplot(means_df, aes(x = i_values)) +
  ...
  labs(
    title = "Stability Measure for Graph(Modularity=0.57)",
    x = "Perturbations(%)",
...
legend <- c("Moderate Modularity Graph","null model")
